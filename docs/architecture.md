...
The next substantial split is to move the React component from direct sparse maps onto the workbook engine. The engine already owns sheets, active-sheet state, ranges, named ranges, merged ranges, data validation, commands, undo/redo, CSV/TSV/HTML-table/SpreadsheetML import/export, formula dependency graph utilities, cached recalculation helpers, common formula functions, range formatting, range sorting, filters, controller persistence snapshots, and storage bindings. It still needs automatic recalculation scheduling, richer workbook metadata, and fuller file adapters before it can credibly approach Excel parity.
