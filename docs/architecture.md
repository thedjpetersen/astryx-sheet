...
The next substantial split is to move the React component from direct sparse maps onto the workbook engine. The engine already owns sheets, ranges, named ranges, merged ranges, data validation, commands, undo/redo, CSV/TSV import/export, formula dependency graph utilities, cached recalculation helpers, range formatting, range sorting, filters, and persistence snapshots. It still needs automatic recalculation scheduling, workbook-level metadata, and richer file adapters before it can credibly approach Excel parity.
